<?php
namespace chinapay; 
use chinapay\CertUtil;
use chinapay\Common;
use chinapay\ChinaConfig;
class AcpService
{
	/**
	 *
	...
	public static function updateEncryptCert(&$params)
	{
		...
		if($certType === "01"){
			...
			if (CertUtil::getEncryptCertId() != $certInfo['serialNumber']) {
				$con = new Common;
				$newFileName = $con->getBackupFileName(ChinaConfig::SDK_ENCRYPT_CERT_PATH);
				// 将原证书备份重命名
				if(!copy(ChinaConfig::SDK_ENCRYPT_CERT_PATH, $newFileName)){
					$logger->LogError ('原证书备份失败');
					return -1;
				}
				// 更新证书
				if(!file_put_contents(ChinaConfig::SDK_ENCRYPT_CERT_PATH, $strCert)){
	...
	static function sign(&$params) {
		if($params['signMethod']=='01')	{
			...
			return AcpService::signByCertInfo($params, ChinaConfig::SDK_SIGN_CERT_PATH_W, ChinaConfig::SDK_SIGN_CERT_PWD);
		} else {

			return AcpService::signBySecureKey($params, ChinaConfig::SECUREKEY);
		} 
	}
	...
	static function signByCertInfo(&$params, $cert_path, $cert_pwd) {
		...
		if($params['signMethod']=='01') {
			...
			// 转换成key=val&串
			$con = new Common;
			$params_str = $con->createLinkString ( $params, true, false );
	...
	static function validate($params) {
		...
		if($params['signMethod']=='01')
		{
			...
			$isSuccess = AcpService::validateBySecureKey($params, ChinaConfig::SECUREKEY);
		} 
		return $isSuccess;
	}
	...
	static function post($params, $url) {
		// $logger = LogUtil::getLogger();
		$con = new Common;
		$opts = $con->createLinkString ( $params, false, true );
		...
		curl_setopt ( $ch, CURLOPT_HTTPHEADER, array ('Content-type:application/x-www-form-urlencoded;charset=UTF-8' ) );
				...
				curl_close ( $ch );
				$con = new Common;
				$result_arr = $con->convertStringToArray ( $html );
				return $result_arr;
	}
...
eot;
	...
	static function getCustomerInfo($customerInfo) {
		...
		$con = new Common;
		return base64_encode ( "{" . $con->createLinkString ( $customerInfo, false, false ) . "}" );
	}
	...
	static function getCustomerInfoWithEncrypt($customerInfo) {
		...
		if( count ($encryptedInfo) > 0 ){
			...
			$encryptedInfo = $con->createLinkString ( $encryptedInfo, false, false );
			$encryptedInfo = AcpService::encryptData ( $encryptedInfo, ChinaConfig::SDK_ENCRYPT_CERT_PATH );
	...
	static function parseCustomerInfo($customerInfostr) {
		...
		$con = new Common;
		$customerInfo = $con->parseQString($customerInfostr);
		if(array_key_exists("encryptedInfo", $customerInfo)) {
			...
			$encryptedInfo = $con->parseQString($encryptedInfoStr);
	...
	static function getEncryptCertId() {
		$cert_path=ChinaConfig::SDK_ENCRYPT_CERT_PATH;
	...
	static function decryptData($data, $cert_path=null, $cert_pwd=null) {
		...
		if( $cert_path == null ) {
			$cert_path = ChinaConfig::SDK_SIGN_CERT_PATH_W;
			$cert_pwd  = ChinaConfig::SDK_SIGN_CERT_PWD;
		}
