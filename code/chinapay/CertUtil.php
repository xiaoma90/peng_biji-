...
//         $cert->certId = $certId;       
    ...
    {
    	if( $certPath == null ) {
    		$certPath = ChinaConfig::SDK_SIGN_CERT_PATH_W;
    		$certPwd = ChinaConfig::SDK_SIGN_CERT_PWD;
    	}
...
// 	    $cert->certId = $certId;
    ...
    public static function verifyAndGetVerifyCert($certBase64String){
		...
		if (ChinaConfig::MIDDLECERT_PATH === null || ChinaConfig::ROOTCERT_PATH === null){
		...
		if(strtolower(ChinaConfig::IFVALIDATECNNAME) == "true"){
			if (COMPANY != $cn){
		...
		$result = openssl_x509_checkpurpose($certBase64String, X509_PURPOSE_ANY, array(ChinaConfig::ROOTCERT_PATH, ChinaConfig::MIDDLECERT_PATH));
		if($result === FALSE){
    ...
    private static function initVerifyCerts($cert_dir=null) {
    	...
    	if( $cert_dir == null ) {
    		$cert_dir = ChinaConfig::VALIDATECERT_DIR;
    	}
